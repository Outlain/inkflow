# Inkflow Resolved Issue History

This file is a troubleshooting reference, not an active todo list. Entries are kept for root-cause history and regression debugging after the underlying issue has been fixed.

For current release status, see [`../status/release-gates.md`](../status/release-gates.md).
